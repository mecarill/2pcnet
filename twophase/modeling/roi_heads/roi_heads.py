...
class StandardROIHeadsPseudoLab(StandardROIHeads):
    ...
    def forward(
        ...
        del images
        if (self.training and compute_loss) or branch == 'consistency_target': # apply if training loss
            assert targets
            # 1000 --> 512
        ...
        if (self.training and compute_loss) or compute_val_loss:
            ...
            if branch == 'consistency_target':
                pred_instances, pred_index = self._forward_box(
                    features, proposals, compute_loss, compute_val_loss, branch
                )
                return pred_instances, proposals, pred_index
            
            if branch == 'unsup_data_consistency':
                pred_instances, predictions = self._forward_box(
                    features, proposals, compute_loss, compute_val_loss, branch,proposal_index
                )
    ...
    def _forward_box(
        ...
        if (
            ...
            # unsup_data_weak goes here
            if branch == 'unsup_data_consistency':
                pred_instances, pred_index = self.box_predictor.inference(predictions, proposals,proposal_index=proposal_index)
                return pred_instances, predictions
